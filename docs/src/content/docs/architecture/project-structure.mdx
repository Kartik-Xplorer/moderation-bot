...
- alita/ **Core application code**
  ...
  - modules/ **Command handlers (features)**
    ...
    - aispam.go Per-chat AI spam filter (TypeSafe Jev)
    - aispam_jev.go Jev client for the AI spam filter
...
### `/alita/utils/chat_status/` - Permission Checking <Badge variant="warning">Security</Badge>
...
- Re-exports model types (`db.User = models.User`) and message-type constants (`db.TEXT`…`db.VIDEO_NOTE`). Cache helpers and TTL constants live in `alita/db/cache/` — they are **not** re-exported from `db.go`.
